...
(function() {
    ...
    function ba(f, d) {
        ...
        for (var b; c.length && (b = c.shift());) !c.length && d !== n ? e[b] = d : e = e[b] ? e[b] : e[b] = {}
    };
    ...
    K.prototype.b = function(f, d, c) {
        ...
        if (8 > d + a) g = g << d | f, a += d;
            ...
            for (m = 0; m < d; ++m) g = g << 1 | f >> d - m - 1 & 1, 8 === ++a && (a = 0, e[b++] = L[g], g = 0, b === e.length && (e = ca(this)));
        e[b] = g;
        this.buffer = e;
        this.e = a;
        this.index = b
    ...
    ia.prototype.push = function(f, d) {
        ...
        for (b[this.length++] = f; 0 < c;)
            if (e = this.getParent(c), b[c] > b[e]) a = b[c], b[c] = b[e], b[e] = a, a = b[c + 1], b[c + 1] = b[e + 1], b[e + 1] = a, c = e;
            else break;
        return this.length
    };
    ia.prototype.pop = function() {
        var f, d, c = this.buffer,
            e, b, a;
        d = c[0];
        f = c[1];
        this.length -= 2;
        ...
        for (a = 0;;) {
            ...
            if (c[b] > c[a]) e = c[a], c[a] = c[b], c[b] = e, e = c[a + 1], c[a + 1] = c[b + 1], c[b + 1] = e;
            else break;
            a = b
        }
        return {
        ...
        switch (this.d) {
            case 0:
                ...
                for (e = b.length; c < e;) {
                    ...
                    var a = d,
                        ...
                        h = this.c;
                    if (C) {
                        for (l = new Uint8Array(this.a.buffer); l.length <= h + a.length + 5;) l = new Uint8Array(l.length << 1);
                        l.set(this.a)
                    }
                    m = g ? 1 : 0;
                    l[h++] = m | 0;
            ...
            case 1:
                ...
                for (fa = s.length; x < fa; x++)
                    if (z = s[x], K.prototype.b.apply(q, T[z]), 256 < z) q.b(s[++x], s[++x], w), q.b(s[++x], 5), q.b(s[++x], s[++x], w);
                    else if (256 === z) break;
                this.a = q.finish();
            ...
            case la:
                var B = new K(C ? new Uint8Array(this.a.buffer) : this.a, this.c),
                    ta, J, O, P, Q, La = [16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15],
                    X, ua, Y, va, ga, ja = Array(19),
                    wa, R, ha, y, xa;
                ta = la;
                B.b(1, 1, w);
                B.b(ta, 2, w);
                ...
                var ya = O,
                    ...
                    r, G, v, Z, E = new(C ? Uint32Array : Array)(316),
                ...
                for (Z = F.length; r < Z; r += G) {
                    ...
                    if (0 === F[r])
                        if (3 > v)
                            ...
                            for (; 0 < v;) A = 138 > v ? v : 138, A > v - 3 && A < v && (A = v - 3), 10 >= A ? (E[D++] = 17, E[D++] = A - 3, H[17]++) : (E[D++] = 18, E[D++] = A - 11, H[18]++), v -= A;
                    else if (E[D++] = F[r], H[F[r]]++, v--, 3 > v)
                        for (; 0 < v--;) E[D++] = F[r], H[F[r]]++;
                    else
                        for (; 0 < v;) A = 6 > v ? v : 6, A > v - 3 && A < v && (A = v - 3), E[D++] = 16, E[D++] = A - 3, H[16]++, v -= A
                }
                ...
                for (Ca = J.length; I < Ca; ++I)
                    if ($ = J[I], B.b(Da[$], Ea[$], w), 256 < $) B.b(J[++I], J[++I], w), ma = J[++I], B.b(Fa[ma], Ga[ma], w), B.b(J[++I], J[++I], w);
                    else if (256 === $) break;
                this.a = B.finish();
                this.c = this.a.length;
                break;
    ...
    var sa = function() {
            ...
            var d = [],
                c, e;
            for (c = 3; 258 >= c; c++) e = f(c), d[c] = e[2] << 24 |
                e[1] << 16 | e[0];
            return d
        }(),
    ...
    function oa(f, d) {
        ...
        var e, b, a, g, m, k = {},
            p, t, u, l = C ? new Uint16Array(2 * d.length) : [],
            h = 0,
            q = 0,
    ...
    function Ja(f, d, c) {
        ...
        var b = new(C ? Uint16Array : Array)(c),
            ...
            l, h, q, s, x;
        b[c - 1] = d;
        for (h = 0; h < c; ++h) t < u ? a[h] = 0 : (a[h] = 1, t -= u), t <<= 1, b[c - 2 - h] = (b[c - 1 - h] / 2 | 0) + d;
        b[0] = a[0];
        m[0] = Array(b[0]);
        k[0] = Array(b[0]);
        for (h = 1; h < c; ++h) b[h] > 2 * b[h - 1] + a[h] && (b[h] = 2 * b[h - 1] + a[h]), m[h] = Array(b[h]), k[h] = Array(b[h]);
        for (l = 0; l < d; ++l) g[l] = c;
        ...
        for (h = c - 2; 0 <= h; --h) {
            s = l = 0;
            x = p[h + 1];
            for (q = 0; q < b[h]; q++) s = m[h + 1][x] + m[h + 1][x + 1], s > f[l] ? (m[h][q] = s, k[h][q] = d, x += 2) : (m[h][q] = f[l], k[h][q] = l, ++l);
            p[h] = 0;
            1 === a[h] && e(h)
        }
        return g
    ...
    Ka.prototype.f = function() {
        ...
        if ("string" === typeof m) {
            var k = m.split(""),
                p, t;
            p = 0;
            for (t = k.length; p < t; p++) k[p] =
                (k[p].charCodeAt(0) & 255) >>> 0;
            m = k
        }
    ...
    ba("Deflate", Ka);
    ba("Deflate.compress", function(f, d) {
        return (new Ka(f, d)).f()
    });
    ba("Deflate.prototype.compress", Ka.prototype.f);
    var Ma = {
            NONE: V.NONE,
            FIXED: V.h,
            DYNAMIC: V.g
    ...
    if (Object.keys) Na = Object.keys(Ma);
        ...
        for (Oa in Na = [], W = 0, Ma) Na[W++] = Oa;
    W = 0;
    for (Pa = Na.length; W < Pa; ++W) Oa = Na[W], ba("Deflate.CompressionType." + Oa, Ma[Oa]);
}).call(this);
